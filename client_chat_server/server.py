...
class ChatServer:
    ...
    async def client_connected(self, reader: StreamReader, writer: StreamWriter):
        ...
        if command == b'CONNECT':
            ...
            logging.error('An invalid command has been received from the client, disconnecting.')
            writer.close()
    ...
    async def _on_connect(self, username: str, writer: StreamWriter):
        writer.write(f'Welcome! Users connected: {len(self._username_to_writer)}')
        await writer.drain()
    ...
    async def _remove_user(self, username: str):
        ...
        try:
            ...
            logging.exception('Error when closing the client writer, ignored.',
                          exc_info=e)
    ...
    async def _listen_for_messages(self, username: str, reader: StreamReader):
        try:
            ...
            logging.exception('Error reading data from the client', exc_info=e)
    ...
    async def _notify_all(self, message: str):
        ...
        for username, writer in self._username_to_writer.items():
            try:
                ...
                logging.exception('Error writing data to the client.', exc_info=e)
